...
# --- CONCEPTUAL OPENX/ROBO-DM DATASET ---
def generate_openx_data(num_cycles=50000):
    """
    Generates a synthetic, large-scale dataset for a versatile robotics task.
    This conceptually represents data from a large-scale, open-source dataset like OpenX.
    - Material Stress, Structural Integrity, and Environmental Load as inputs.
    - A multi-class label for a "Construction" action (Monitor, Reinforce, Weld, Assemble, Debug).
    """
    cycles = np.arange(1, num_cycles + 1)

    # Simulate a stable production environment with occasional fluctuations
    material_stress = 1000 + 100 * np.sin(cycles * 0.001) + np.random.normal(0, 20, num_cycles)
    structural_integrity = 0.95 + 0.03 * np.cos(cycles * 0.002) + np.random.normal(0, 0.01, num_cycles)
    environmental_load = 500 + 200 * np.sin(cycles * 0.003) + np.random.normal(0, 30, num_cycles)

    # The action is a multi-class label (0, 1, 2, 3 or 4)
    action = np.zeros(num_cycles)  # 0: Monitor
    action[(material_stress > 1100) | (structural_integrity < 0.9)] = 1  # 1: Reinforce
    action[environmental_load > 650] = 2  # 2: Weld
    action[(material_stress < 900) | (structural_integrity > 0.98)] = 3  # 3: Assemble
    action[(material_stress < 950) & (environmental_load < 400)] = 4  # 4: Debug
...
class RoboticsTransformer(nn.Module):
    def __init__(self, input_dim):
        ...
        self.output_layer = nn.Linear(128, 5)  # 5 outputs for multi-class actions
...
class ChildPerceptron(nn.Module):
    def __init__(self, input_dim):
        ...
        self.output_layer = nn.Linear(64, 5)  # 5 outputs for multi-class actions
...
# --- DATA PREPARATION ---
X = df_openx[['Material Stress', 'Structural Integrity', 'Environmental Load']].values
y = df_openx['Construction Action'].values
...
print("--- TRAINING THE PARENT ROBOTICS TRANSFORMER on OpenX Data ---")
